                                       ...
                                       size_t val_base, size_t fac_base)
{
  ...
  for (size_t i = v_start; i < v_end; ++i)
  {
    ...
    if (v.adjacent_factors)//如果有向量的factor
    {
      // sort by <value, fid>
...
inline void FactorGraph::sgd_on_factor(size_t factor_id, 
                                       double stepsize,
                                       size_t vid,
                                       size_t evidence_value,
                                  ...
                                  bool is_noise_aware)
{
  ...
  else
  {
    ...
    for (size_t val = 0; val < variable.internal_cardinality(); ++val)
    {
      ...
      if (!is_noise_aware && val != variable.assignment_dense)
        continue;

      const VariableToFactor &ev = values[variable.var_val_base + val];
      if (is_noise_aware && is_linear_zero(ev.truthiness))
        continue;
