...
class PatternMgr:
    ...
    def match(self, pattern, that, topic):
        ...
        # 将输入传递给递归调用
        patMatch, template = self._match(input_.split(), thatInput.split(),
...
       - “topicstar”：与topic模式中的一个星号匹配。        """
        ...
        for i in range(len(words)):
            ...
            if not foundTheRightStar:
                if patMatch[j] in [self._STAR,
                                   self._UNDERSCORE]:  #we got a star
                    numStars += 1
                    if numStars == index:
                        # 这个是我们关心的那个 star .
    ...
    def _match(self, words, thatWords, topicWords, root):
        ...
        if len(words) == 0:
            ...
            if len(thatWords) > 0:
                ...
                try:
                    pattern, template = self._match(thatWords, [], topicWords,
                                                    root[self._THAT])
                    if pattern != None:
                ...
                try:
                    pattern, template = self._match(topicWords, [], [],
                                                    root[self._TOPIC])
                    if pattern != None:
        ...
        if self._UNDERSCORE in root:
            ...
            for j in range(len(suffix) + 1):
                suf = suffix[j:]
                pattern, template = self._match(suf, thatWords, topicWords,
