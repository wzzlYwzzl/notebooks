"""
这里的实现没有借助tensorflow中提供的高级RNN的接口，而是使用了相对低级的接口完成计算图的构建
"""
...
truncated_backprop_length = 15  # 序列长度
state_size = 4
num_classes = 2  # 序列标注的类别数量
echo_step = 3
batch_size = 5
...
for current_input in inputs_series:
    ...
    input_and_state_concatenated = tf.concat(
        [current_input, current_state], axis=1)
...
# 计算损失函数
logits_series = [tf.matmul(state, W2) for state in states_series]
...
def plot(loss_list, predictions_series, batchX, batchY):
    ...
    for batch_series_idx in range(5):
        ...
        plt.bar(left_offset,
                batchX[batch_series_idx, :], width=1, color="blue")
        plt.bar(left_offset, batchY[batch_series_idx, :]
                * 0.5, width=1, color="red")
        plt.bar(left_offset, single_output_series *
                0.3, width=1, color="green")
